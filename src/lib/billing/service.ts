...
import { db } from "../db";
import { billing, stripeEvent } from "../db/schema";
import { appUrl, stripeEnv } from "../env";
...
export async function requirePaid(userId: string) {
  ...
  if (!hasPaidAccess(state, stripeEnv().STRIPE_PRO_PRICE_ID)) {
    throw new HttpError(403, "A current Pro subscription is required.");
  }
  return state;
}
...
export async function portal(userId: string) {
  ...
  const [row] = await db()
    ...
    .where(eq(billing.userId, userId));
  if (!row?.customerId) {
    throw new HttpError(400, "Choose Pro before opening billing.");
  }
  return (
...
export async function processStripeEvent(event: Stripe.Event) {
  if (event.livemode !== stripeEnv().STRIPE_LIVE_MODE) {
    throw new HttpError(400, "Stripe mode mismatch.");
  }
