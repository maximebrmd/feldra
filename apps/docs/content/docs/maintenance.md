...
description: "Keep your project independent and carry useful fixes forward."
sidebar:
  order: 10
---
...
Customize `packages/config/index.ts`, app copy, metadata and the example notes. Each project owns its provider resources and environment. Never distribute `.env.local` or real credentials. Generated projects have no dependency on create-saas-keel and do not automatically change when it updates.
