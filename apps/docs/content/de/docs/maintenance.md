...
Passe `packages/config/index.ts`, die App-Texte, Metadaten und Beispielnotizen an. Jedes Projekt verwaltet seine eigenen Provider-Ressourcen und seine eigene Umgebung. Gib niemals `.env.local` oder echte Zugangsdaten weiter. Generierte Projekte haben keine Abhängigkeit von create-feldra und ändern sich nicht automatisch, wenn es aktualisiert wird.
