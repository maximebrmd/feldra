...
- Storage overlays live in `packages/feldra/variants/storage/{r2,blob}` and are applied independently through `packages/feldra/bin/apply-storage.mjs`; R2 is the default. Their provider-only lockfiles are hashed into `template-manifest.json`, while `create.mjs` resolves the final lockfile after auth, storage, and docs overlays are combined.
