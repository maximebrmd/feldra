...
3. Run `npm ci`, `npm run check`, `npm run test:database`, and `npm run test:browser`. Browser tests install Chromium if missing. Docker is required for the latter two.
...
5. Run `npm run initializer:test`. It packs the base, runs the **tarball** initializer in a temporary directory with spaces, checks contents/naming/git/environment, checks overwrite refusal, then runs the generated project's checks and local database/browser tests. It also inspects tarball entries and rejects forbidden paths.
6. Inspect `tar -tzf create-saas-keel-0.1.0.tgz`, `npm pack --dry-run ./initializer`, and the generated `initializer/template-manifest.json`. Only bin, bundled source, licenses, manifest and README belong in the release; no secrets, node_modules, Git history, agent skills or research sources.
...
```sh
npm publish ./create-saas-keel-0.1.0.tgz --access public
```
