...
- **Blume:** inspected `packages/blume/package.json`, `src/core/schema.ts`, and `bin/blume.mjs`. It is an Astro documentation framework with a published Node CLI, not a SaaS application using the same stack. Adopted the idea of a clear entry point and deliberate package contents. Generated SaaS apps (`apps/web`, `apps/app`) have no Blume dependency. Generated projects include a docs app: Blume by default, or Mintlify or Fumadocs when selected. The Feldra product documentation site uses Blume directly, as described below.
...
Appwrite Auth (`node-appwrite` 29.0.0) follows the official SSR pattern: an admin client with a Sessions write API key creates sessions, and a per-request session client calls `setSession` with the httpOnly cookie value (`session.secret`, not `session.$id`). Sources: https://appwrite.io/docs/products/auth/server-side-rendering, https://appwrite.io/docs/tutorials/nextjs-ssr-auth, and https://www.next-forge.com/docs/migrations/authentication/appwrite. Appwrite is an authentication overlay only; Postgres remains Neon or Supabase.

### Optional feature flags

Read the current next-forge feature-flags guide at https://www.next-forge.com/docs/packages/flags and the authoritative Flags SDK references at https://flags-sdk.dev/docs/api-reference/frameworks/next and https://github.com/vercel/flags. Feldra's `--flags vercel` overlay uses the provider-agnostic `flags` SDK with a safe environment-backed example flag and the protected `/.well-known/vercel/flags` discovery route. It does not add PostHog or provision a provider; generated projects can add an adapter later. The overlay is kept under `packages/feldra/variants/flags` so the default generated project remains unchanged.
