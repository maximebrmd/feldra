...
One Next.js app, no workspace tooling. Domain code in `src/lib` is server-only. Only small interactive forms import client code. Better Auth sessions are read on every protected request; no cookie cache extends revoked sessions. Protected layouts handle navigation, but every page and route independently authenticates. Email verification is required for app access. Onboarding is a UX step, not a paid-access gate.

All note mutations use strict Zod schemas. Owner IDs come exclusively from the verified session. Updates/deletes include both ID and owner in the SQL predicate and return 404 for missing or foreign rows. Reads filter the owner. Same-origin checks protect custom mutating endpoints; Better Auth handles its own CSRF and trusted origins. Forms have explicit labels, pending buttons, live status/errors, empty states, focus rings and a skip link.
...
Remove `src/lib/notes.ts`, `src/components/note-editor.tsx`, `src/app/api/notes`, and replace the dashboard notes view. Remove the settings export link and choose a real server-enforced Pro feature. Remove the `note` table from schema, generate a new migration, review data loss before applying it, and replace example tests. Keep auth and billing tests. Do not rewrite migrations already applied to derived projects.
