...
The initializer can instead generate Clerk with `--auth clerk` or Auth.js with `--auth authjs`. Those projects receive a different version of this guide, provider-specific session checks, and omit Better Auth and Resend. This is a new-project choice, not an automatic migration of existing users.
