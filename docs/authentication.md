...
The initializer can instead generate Clerk with `--auth clerk`, Auth.js with `--auth authjs`, Supabase Auth with `--auth supabase`, or Appwrite Auth with `--auth appwrite`. Those projects receive a different version of this guide, provider-specific session checks, and omit Better Auth and Resend. Authentication is independent of `--database`; Supabase Auth still needs a Supabase project URL and publishable key. These are new-project choices, not an automatic migration of existing users.
