...
Create a separate Neon or Supabase project/database, Clerk application and keys, and Stripe sandbox (and later separate live product/prices/webhook) per SaaS. Never copy this project's credentials, databases or Stripe resource IDs into another project. Separate local/test/production resources; previews must not use production data. None are provisioned by the initializer.
...
Set APP_URL and WEB_URL on both projects to their exact production HTTPS origins. Marketing needs only these origins; set database, Clerk and Stripe credentials **only on the application project**. Use a separate production Clerk instance, production Postgres database, and matching Stripe live key/price/mode. Authentication cookies belong to the application origin; cross-subdomain cookies and permissive CORS are not needed. Marketing links navigate to the app for signup, login and subscription actions.
