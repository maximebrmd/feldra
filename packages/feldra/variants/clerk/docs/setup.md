...
Create a separate Neon or Supabase project/database, R2 bucket or Blob store, Clerk application and keys, and Stripe sandbox (and later separate live product/prices/webhook) per SaaS. Never copy this project's credentials, storage, databases, or Stripe resource IDs into another project. Separate local/test/production resources; previews must not use production data. None are provisioned by the initializer.
...
| Variable | Value |
| --- | --- |
| APP_URL | Exact origin `http://localhost:3001` locally; HTTPS authenticated-app origin in production, without paths/query. Used for authorized parties and billing redirects. |
...
| NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY | Public Clerk key (`pk_test_…` or `pk_live_…`). Rebuild after changes. |
| CLERK_SECRET_KEY | Server-only Clerk key (`sk_test_…` or `sk_live_…`). |
...
Follow [database setup](databases.md) and [Clerk authentication](authentication.md). Configure your keys, run `npm run db:migrate`, then `npm run dev`. Require email verification in Clerk. Test signup, verification, login, password recovery, logout and onboarding against your own development instance before launch. This project never provisions Clerk automatically.
...
Set APP_URL and WEB_URL on both projects to their exact production HTTPS origins. Marketing needs only these origins; set database, Clerk, storage, and Stripe credentials **only on the application project**. Use a separate production Clerk instance, production Postgres database, isolated production storage, and matching Stripe live key/price/mode. Authentication cookies belong to the application origin; cross-subdomain cookies and permissive CORS are not needed. Marketing links navigate to the app for signup, login and subscription actions.
...
Fixture tests use real local Postgres and application route handlers. Clerk, storage, and Stripe network boundaries are replaced only inside tests. Signed payloads use Stripe's real signature utility. They do not claim successful live delivery, storage, or payments. Live Neon/Supabase connectivity, Clerk delivery, R2 or Blob operations, Stripe hosted Checkout/portal, and hosting need your project credentials and manual verification.
