...
export function AuthForm({ mode, invalid }: { mode: Mode; invalid?: boolean }) {
  const fieldId = useId();
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
        ...
        case "signup": {
          const { error: signUpError } = await supabase.auth.signUp({
            ...
            options: {
              ...
              emailRedirectTo: `${origin}/api/auth/callback?next=/login`,
            },
            password,
          });
      ...
      if (resultError) {
        setError(
          resultError.message ?? "Unable to continue. Please try again."
        );
        return;
      }
      if (mode === "login") {
    ...
    <div className="shell max-w-lg py-16">
      ...
      {mode === "verify" && (
        <p className="mb-6 text-muted-foreground text-sm">
          Open the verification link before signing in. Need another email?
