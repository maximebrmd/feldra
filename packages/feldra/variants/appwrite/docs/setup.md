...
Every variable in `.env.example` is consumed; there are no browser-exposed secrets beyond the Appwrite endpoint and project ID, which Appwrite treats as public. Both Next app configs load root `.env.local`; migrations read the same file. Production host environment variables take precedence. Validation happens when the relevant service is invoked, allowing a credential-free production build. Invalid/missing providers fail closed; no successful email or billing response is invented.

| Variable | Value |
| --- | --- |
| APP_URL | Exact origin `http://localhost:3001` locally; HTTPS authenticated-app origin in production, without paths/query. Used for verification/reset links and billing redirects. |
...
| NEXT_PUBLIC_APPWRITE_ENDPOINT | Appwrite API origin including path, typically `https://cloud.appwrite.io/v1` or `https://<REGION>.cloud.appwrite.io/v1`. HTTPS required except HTTP localhost. Rebuild after changes. |
| NEXT_PUBLIC_APPWRITE_PROJECT_ID | Appwrite project ID from the console. Rebuild after changes. |
| APPWRITE_API_KEY | Server-only API key with the Sessions write scope. Never expose it to the browser. |
...
The API key used to create sessions bypasses Appwrite's client IP rate limits. Keep it server-only. Login, signup, password recovery and verification resend are limited per client IP in the `rate_limit` table (5 sign-in/sign-up and 3 recovery/resend attempts per 60 seconds). The trusted client IP header is `x-vercel-forwarded-for`, which Vercel supplies. Never trust `X-Forwarded-For`; if no trusted IP is available, these routes share a conservative per-path bucket.
